...
class CupHandleDetector:
    ...
    def validate_rim_levels(self, peak_a_price, peak_c_price, breakout_e_price=None):
    ...
    def detect_accumulation_base(self, df, resistance_level):
        ...
        timeframe_minutes = self.detect_timeframe(self._processed_df) if hasattr(self, 'detect_timeframe') else 15
    ...
    def detect_resistance_levels(self, df, lookback_bars=None):
        ...
        if lookback_bars is None:
            timeframe_minutes = self.detect_timeframe(self._processed_df)
    ...
    def remove_duplicate_patterns(self, patterns):
        ...
        for pattern in patterns:
            ...
            for existing in unique_patterns:
                # Check for EXACT timestamp matches (strict/relaxed duplicates)
                exact_match = (
                    pattern['peak_a'] == existing['peak_a'] and
                    pattern['peak_c'] == existing['peak_c'] and
                    pattern['breakout_e'] == existing['breakout_e']
                )
                
                if exact_match:
                    print(f"   🔄 EXACT DUPLICATE: {pattern.get('pattern_id', 'unknown')} matches {existing.get('pattern_id', 'unknown')}")
                    is_duplicate = True
                    break
                    
                # Check if similar period (within 1 hour) - for different patterns
                time_diff = abs((pattern['peak_a'] - existing['peak_a']).total_seconds())
                if time_diff < 3600:  # Less than 1 hour apart
                    print(f"   🔄 SIMILAR TIMING: {pattern.get('pattern_id', 'unknown')} similar to {existing.get('pattern_id', 'unknown')}")
                    is_duplicate = True
                    break
    ...
    def find_breakout_after_handle(self, df, handle_end, resistance_level, peak_a_price, peak_c_price, peak_c_time):
        ...
        current_atr = self.calculate_atr(self._processed_df, 20).iloc[-1]  # You already have this method
        ...
        for i in range(search_start_idx, search_end_idx):
            ...
            if current_high >= breakout_threshold * 0.999:
                breakout_found = True
    ...
    def detect_handle_formation(self, df, resistance_level, peak_c_time, peak_c_price, peak_a_time, trough_b_time):
        ...
        print(f"🔍 HANDLE ATTEMPT: resistance=${resistance_level:.2f}, peak_c=${peak_c_price:.2f}")
        self.detection_stats['handle_analysis']['accumulations_tested'] += 1
        
        peak_c_idx = df.index.get_loc(peak_c_time)
   
        search_start_idx = peak_c_idx + 3
        if search_start_idx >= len(df) - 30:  # ← CHANGED: 5 to 30
            print(f"         ❌ INSUFFICIENT DATA: Only {len(df) - search_start_idx} bars left after peak_c")
        ...
        if search_start_idx >= len(df) - 30:
            print(f"         ❌ REJECTION REASON: insufficient_data (only {len(df) - search_start_idx} bars left)")
            self.detection_stats['handle_analysis']['rejection_reasons']['insufficient_data'] += 1
            return []
        ...
        for i in range(handle_start_idx + min_handle_bars, min(len(search_window), max_handle_bars_inner)):
            ...
            if len(potential_handle) < min_handle_bars:
                print(f"         ❌ HANDLE REJECTED: Too short - {len(potential_handle)} bars < {min_handle_bars} minimum")
                continue

            handle_low_price = potential_handle['low'].min()

            if handle_low_price >= peak_c_price:
                print(f"         ❌ HANDLE REJECTED: No pullback - handle_low ${handle_low_price:.2f} >= peak_c ${peak_c_price:.2f}")
                continue
            ...
            if not handle_depth_valid:
                ...
                print(f"         ❌ HANDLE REJECTED: Depth validation failed - {depth_msg}")
                continue
            ...
            if (min_handle_bars <= duration <= max_handle_bars_inner):
                ...
                if not is_valid_rim:
                    print(f"         ❌ HANDLE REJECTED: Rim violation - {rim_reason}")
                    continue
        ...
        print(f"         🔍 HANDLE DEBUG: Found {len(search_window)} bars to search")
        print(f"         🔍 HANDLE DEBUG: min_handle_bars={min_handle_bars}, max_handle_bars_inner={max_handle_bars_inner}")
        print(f"         🔍 HANDLE DEBUG: peak_c_price=${peak_c_price:.2f}")
    ...
    def validate_handle_never_above_rim(self, df, handle_start, handle_end, rim_level):
        ...
        if max_handle_high > rim_level * 1.02:
    ...
    def validate_handle_depth_ratio(self, cup_peak_price, cup_bottom_price, handle_low_price):
        ...
        max_handle_ratio = self.config.get('max_handle_depth_pct', 75.0) / 100.0  
        print(f"🔧 HANDLE DEPTH DEBUG: depth_ratio={depth_ratio:.3f}, max_allowed={max_handle_ratio:.3f}, config_val={self.config.get('max_handle_depth_pct', 75.0)}")
        
        if depth_ratio <= 0.25:
            return True, "OPTIMAL"
        elif depth_ratio <= max_handle_ratio:  # ← THIS SHOULD USE max_handle_ratio, NOT 0.70
    ...
    def create_institutional_pattern(self, resistance, accumulation, handle, breakout_time, df, actual_peak_c_time, price_col='close_smooth', validated_peak_a_time=None, validated_peak_a_price=None):
        ...
        if validated_peak_a_time is not None and validated_peak_a_price is not None:
            peak_a_time = validated_peak_a_time
            peak_a_price = validated_peak_a_price
        else:
            # Fallback to accumulation start (should rarely happen)
        ...
        if handle['end'] >= breakout_time:
            print(f"❌ INVALID: Handle {handle['end']} >= breakout {breakout_time}")
            return None
    ...
    def find_optimal_right_rim(self, df, accumulation_start, accumulation_end, resistance_level, left_rim_price, trough_time):
        ...
        try:
            ...
            for peak_time, peak_row in atr_candidates:  # ← ONLY CHANGE: use atr_candidates instead of actual_peaks.iterrows()
                ...
                symmetry_score = max(0, 100 - price_diff_pct * 60)
    ...
    def filter_patterns(self, patterns):
        ...
        # STEP 2: Calculate ALL variables once, outside the loop
        if hasattr(self, '_processed_df') and self._processed_df is not None:
            ...
            min_cup_depth_pct = 0.3
            
        volatility_multiplier = min(1.2, 1 + recent_volatility * 2)

        # Adaptive thresholds based on market conditions
        base_symmetry = self.config.get('min_cup_symmetry', 0.02)
        base_roundness = self.config.get('min_cup_roundness', 0.02)

        min_cup_symmetry = max(0.03, base_symmetry / volatility_multiplier)
        ...
        for p in complete_patterns:  # ← Use complete_patterns, not original patterns
            ...
            if p.get('quality_score', 0) < self.config['min_quality_score']:
                continue
            pattern_min_roundness = min_cup_roundness 
            pattern_min_symmetry = min_cup_symmetry

            cup_depth = p.get('cup_depth_pct', 0)
            if cup_depth >= 8.0:  # Very deep cups (8%+) 
                min_cup_symmetry *= 0.7  # Allow 30% lower symmetry requirement
            elif cup_depth >= 5.0:  # Deep cups (5-8%)
                min_cup_symmetry *= 0.85

            
            handle_depth_ratio = p.get('handle_depth_ratio_to_cup', 0)
            if 0.15 <= handle_depth_ratio <= 0.35:  # Optimal handle depth range
                min_cup_roundness *= 0.5  # Allow 20% lower roundness requirement
                print(f"   🎯 QUALITY BONUS: Excellent handle depth {handle_depth_ratio:.2f} - relaxed roundness to {min_cup_roundness:.2f}")

               # Use pattern-specific thresholds in validation
            if p.get('cup_symmetry', 0) < pattern_min_symmetry:
                print(f"   ❌ FILTERED: Cup too asymmetrical {p.get('cup_symmetry', 0):.2f} < {pattern_min_symmetry:.2f}")
                continue
                
            if p.get('cup_roundness', 0) < pattern_min_roundness:
                print(f"   ❌ FILTERED: Cup too V-shaped {p.get('cup_roundness', 0):.2f} < {pattern_min_roundness:.2f}")
                continue

            cup_duration_hours = p.get('cup_duration_min', 0) / 60
            
            if timeframe_minutes <= 15:  # Intraday timeframes
                max_cup_hours = 120  # Maximum 72 hours for 15min patterns
                if cup_duration_hours > max_cup_hours:
    ...
    def get_adaptive_parameters(self, df, timeframe_minutes):
        ...
        recent_data = self._processed_df.tail(100)  # Last 100 bars
    ...
    def get_strict_config(self):
        ...
        base_config.update({
            ...
            "prominence": 0.001,
            "min_quality_score": 55,           # ← Use your custom value
            "min_cup_depth": 0.003,           # ← Use your custom value
            "min_cup_duration": 60,           # ← Use your custom value
    ...
    def merge_pattern_results(self, strict_patterns, relaxed_patterns):
        ...
        for pattern in strict_patterns:
            pattern_key = (pattern['peak_a'], pattern['peak_c'], pattern['breakout_e'])
            pattern['pattern_id'] = f"strict_{len(all_patterns)}"
            all_patterns.append(pattern)
            seen_patterns.add(pattern_key)

        # Add only NEW relaxed patterns
        for pattern in relaxed_patterns:
            ...
            if pattern_key not in seen_patterns:
                ...
                print(f"   🔄 SKIPPING relaxed duplicate: {pattern_key}")
    ...
    def detect_cup_and_handle_deduped(self, df, extrema_col='extrema', price_col='close_smooth'):
        ...
        processed_accumulations = set()
 
            
        for i, resistance in enumerate(resistance_levels[:resistance_limit]):
                ...
                for j, accumulation in enumerate(accumulation_periods):
                    # ADD THIS: Skip if we've already processed this accumulation period
                    cup_signature = f"{accumulation['start'].strftime('%Y%m%d_%H%M')}_{accumulation['end'].strftime('%H%M')}"
                    if cup_signature in processed_accumulations:
                        print(f"      ⏭️  SKIPPING: Accumulation period already processed")
                        continue
                    processed_accumulations.add(cup_signature)
                ...
                print(f"         📝 ADDED cup signature: {cup_signature}")
                ...
                tolerance = self.config['rim_height_tolerance_pct']

                print(f"🔍 RIM CHECK DEBUG:")
                print(f"   Peak A: ${peak_a_price:.2f}")
                print(f"   Peak C: ${peak_c_price:.2f}")  
                print(f"   Difference: {rim_diff_pct:.3f}%")
                print(f"   Tolerance: {tolerance}%")
                print(f"   Should pass: {rim_diff_pct <= tolerance}")

                if rim_diff_pct > tolerance:
                    print(f"❌ REJECTING: Rim asymmetry {rim_diff_pct:.3f}% > {tolerance}%")
                    continue  # This will now actually skip the pattern
                else:
                    print(f"✅ ACCEPTING: Rim asymmetry {rim_diff_pct:.3f}% <= {tolerance}%")
                ...
                if handles:
                    ...
                    pattern = self.create_institutional_pattern(
                        ...
                        validated_peak_a_time=peak_a_time, validated_peak_a_price=peak_a_price
                    )
        ...
        if timeframe_minutes >= 1440:  # Daily+ data
            ...
            return {
             ...
             return {
                    ...
                    "min_cup_duration": 30,               # ← 5 hours minimum (human labels)
...
if __name__ == "__main__":
    ...
    "enable_formation_first": False,
    # "min_handle_gap_minutes": 150, 
    "max_handle_depth_pct": 75.0,  # Keep existing max
