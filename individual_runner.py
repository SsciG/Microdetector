...
def run_detector_v2_true_individual(df_copy):
    """Run detector_v2.py with NO CONFIG OVERRIDE - let it use its own"""
    from detector_v2 import CupHandleDetector
    
    # NO CONFIG - let detector_v2 use whatever it has internally
    detector = CupHandleDetector()  # Use its default internal config
    
    # Only apply method replacement if it exists
...
def run_detector_v3_true_individual(df_copy):
    """Run detector_v3.py with NO CONFIG OVERRIDE - let it use its own"""
    from detector_v3 import CupHandleDetector
    
    # NO CONFIG - let detector_v3 use whatever it has internally
    detector = CupHandleDetector()  # Use its default internal config
