...
class CupHandleDetector:
    ...
    def calculate_cup_roundness(self, df, cup_start, cup_bottom, cup_end, price_col='close_smooth'):
        ...
        try:
            ...
            if coeffs[0] <= 0.1:  # Not a proper U-shape
                return 0.1
    ...
    def _validate_rim_symmetry(self, peak_a_price, peak_c_price):
        """Validate rim height symmetry"""
        tolerance = self.config.get("rim_height_tolerance_pct", 2.5) / 100
        if max(peak_a_price, peak_c_price) == 0:
            return False
        
        diff = abs(peak_a_price - peak_c_price) / max(peak_a_price, peak_c_price)
    ...
    def create_institutional_pattern(self, resistance, accumulation, handle, breakout_time, df,actual_peak_c_time, price_col='close_smooth'):
        ...
        if breakout_time is None:
         ...
         # NEW: Check for identical timestamps
        timestamps = [
            accumulation['start'],  # peak_a
            None,  # trough_b (calculated later)
            actual_peak_c_time,     # peak_c
            handle['start'],        # handle_d
            breakout_time           # breakout_e
        ]
        
        # Check for identical timestamps in known values
        known_timestamps = [t for t in timestamps if t is not None]
        if len(known_timestamps) != len(set(known_timestamps)):
            print(f"   ❌ REJECTED: Identical timestamps detected")
            return None
    ...
    def filter_patterns(self, patterns):
        ...
        for p in complete_patterns:  # ← Use complete_patterns, not original patterns
            ...
            if timeframe_minutes <= 15:  # Intraday timeframes
                max_cup_hours = 72  # Maximum 72 hours for 15min patterns
                if cup_duration_hours > max_cup_hours:
        ...
        for i, resistance in enumerate(resistance_levels[:resistance_limit]): # Limit to top 5 resistance levels
            ...
            for j, accumulation in enumerate(accumulation_periods):
                ...
                rim_valid, rim_msg = self.validate_no_higher_highs_in_cup(
                    df, peak_a_time, peak_c_time, tolerance_pct=0.5
                )
                if not rim_valid:
                    print(f"         ❌ {rim_msg}")
                    continue
