...
class CupHandleDetector:
    ...
    def detect_cup_and_handle(self, df, extrema_col='extrema', price_col='close_smooth', 
                              breakout_e_price='breakout_e', resistance_level=None):
        ...
        for i, peak_a_time in enumerate(peaks.index[:-1]):
            ...
            for peak_c_time in peaks.index[i+1:]:
                ...
                if not self.validate_rim_levels(peak_a_price, peak_c_price):
                    continue
    ...
    def validate_handle_depth_ratio(self, cup_peak_price, cup_bottom_price, handle_low_price):
        """Validate handle depth ratio - RELAXED for 4H ES"""   
        # STEP 1: Critical check - handle must be above cup bottom
        if handle_low_price <= cup_bottom_price:
            return False, "Handle below cup bottom - INVALID"
        
        # STEP 2: If above cup bottom, allow deeper ratios
        cup_depth = cup_peak_price - cup_bottom_price
        handle_depth = cup_peak_price - handle_low_price
        depth_ratio = handle_depth / cup_depth
        
        # RELAXED ratio check - allow up to 150% if above cup bottom
        if depth_ratio <= 1.5:  # Instead of 0.75
            return True, "Valid handle above cup bottom"
        else:
            return False, f"Handle too deep: {depth_ratio:.2f} ratio"
    ...
    def find_optimal_right_rim(self, df, accumulation_start, accumulation_end, resistance_level, left_rim_price, trough_time):
        ...
        try:
            ...
            for peak_time, peak_row in actual_peaks.iterrows():
                ...
                is_local_high = False  # Initialize before if block
    ...
    def filter_patterns(self, patterns):
        ...
        # STEP 2: Calculate timeframe and volatility once
